...
export const Dialog: DialogComponent = ({ children, defaultOpen = false, open, onOpenChange }: DialogProps) => {
  ...
  const contextValue = React.useMemo(() => ({ isOpen, setIsOpen, closeDialog, animationProgress }), [animationProgress, closeDialog, isOpen, setIsOpen]);
      ...
      {React.Children.map(children, child => {
        if (React.isValidElement(child) && child.type === DialogContent) {
          return React.cloneElement(child, {
            isAnimating,
            setIsAnimating,
          } as Partial<ExtendedDialogContentProps>);
        }
        return child;
      })}
...
const DialogTrigger: React.FC<DialogTriggerProps> = ({ children, asChild }) => {
  ...
  if (asChild && React.isValidElement(children)) {
    const childOnPress = (children.props as PressableProps).onPress;
...
const DialogBackdrop: React.FC<DialogBackdropProps> = ({ children, blurAmount = 20, backgroundColor = 'rgba(0, 0, 0, 0.5)', blurType = 'dark' }) => {
...
const DialogContent: React.FC<ExtendedDialogContentProps> = ({
  ...
  }, [animationProgress, externalIsAnimating, externalSetIsAnimating, onClose, setIsOpen]);
  ...
  }, [dismissible, externalSetIsAnimating]);
  ...
  return (
    <Modal visible={isOpen} transparent animationType='none' statusBarTranslucent onRequestClose={handleRequestClose}>
      <DialogBackdrop blurAmount={backdropBlurAmount} backgroundColor={backdropColor} blurType={backdropBlurType} />
